...
// The collection Claude Code session sync pushes into (see hooks.rs,
// session.rs). Kept in its own file, not credentials.json — logging out
// shouldn't discard the chosen collection, and re-running
// `flockfly init --collection <id>` shouldn't require re-authenticating.
#[derive(Clone, Debug, Deserialize, Serialize)]
#[serde(rename_all = "camelCase")]
...
fn sync_config_path(env: &HashMap<String, String>) -> PathBuf {
    config_dir(env).join("sync-config.json")
}

pub fn load_sync_config(env: &HashMap<String, String>) -> Option<SyncConfig> {
    let contents = fs::read_to_string(sync_config_path(env)).ok()?;
    let config: SyncConfig = serde_json::from_str(&contents).ok()?;
...
pub fn save_sync_config(config: &SyncConfig, env: &HashMap<String, String>) -> io::Result<()> {
    let directory = config_dir(env);
    fs::create_dir_all(&directory)?;
    let path = sync_config_path(env);
